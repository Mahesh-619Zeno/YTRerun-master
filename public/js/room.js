...
function copyTextToClipboard(text, callback) { navigator.clipboard.writeText(text).then(() => { if (callback) { callback(); } }).catch(err => { console.error('Failed to copy text: ', err); alert("Copying failed. Please copy the link from the address bar."); }); }
